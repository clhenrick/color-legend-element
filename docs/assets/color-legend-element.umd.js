...
/**
     ...
     * Copyright 2017 Google LLC
     * SPDX-License-Identifier: BSD-3-Clause
     */;class pt{constructor(t){this.cle=t}setColorScale(){switch(this.cle.scaleType){case"continuous":this.setContinousColorScale();break;case"log10":this.setLogColorScale();break;case"discrete":this.setDiscreteColorScale();break;case"threshold":this.setThresholdColorScale();break;case"categorical":this.setCategoricalColorScale();break;default:this.invalidScaleType(this.cle.scaleType)}}setContinousColorScale(){const{interpolator:t,domain:s,range:r}=this.cle;this.colorScale=t?i.scaleSequential(t).domain(s):i.scaleLinear().range(r).domain(s).interpolate(e.interpolateHcl)}setLogColorScale(){const{interpolator:t,domain:s,range:r}=this.cle;this.colorScale=t?i.scaleSequentialLog(t).domain(s):i.scaleLog().range(r).domain(s).interpolate(e.interpolateHcl)}setDiscreteColorScale(){this.colorScale=i.scaleQuantize().domain(this.cle.domain).range(this.cle.range)}setThresholdColorScale(){const t=this.cle.domain;this.colorScale=i.scaleThreshold().domain(t.slice(1,t.length-1)).range(this.cle.range)}setCategoricalColorScale(){this.colorScale=i.scaleOrdinal().domain(this.cle.domain).range(this.cle.range)}invalidScaleType(t){throw new Error(`invalid property scaletype: ${t}.\n      Must be one of "categorical", "continuous", "discrete", "threshold".`)}}
/**
     * @license
     * Copyright 2017 Google LLC
     * SPDX-License-Identifier: BSD-3-Clause
...
/**
    ...
    >
      ...
      <slot name="footer"></slot>
        ...
        >
          ${t}
        </li>`))}`}renderContinuous(){if("continuous"!==this.cle.scaleType&&"log10"!==this.cle.scaleType||null===this.cle.colorScale)return"";const{colorScale:t,marginTop:i,marginLeft:s,marginRight:r,tickSize:o,width:n,range:h}=this.cle,c=this.cle.marginBottom+o,l=this.cle.height+o,a=t.interpolator?.()||e.piecewise(e.interpolateHcl,h);return H`<image
      x=${s}
      y=${i}
      width=${n-r-s}
      height=${l-i-c}
