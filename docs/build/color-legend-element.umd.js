...
/**
    ...
    >
      ...
      >
        ${t}
      </li>`))}`}renderContinuous(){var t,i;if("continuous"!==this.cle.scaleType||null===this.cle.colorScale)return"";const{colorScale:e,marginTop:n,marginLeft:o,marginRight:r,tickSize:l,width:h,range:c}=this.cle,a=this.cle.marginBottom+l,d=this.cle.height+l,u=(null===(i=(t=e).interpolator)||void 0===i?void 0:i.call(t))||s.piecewise(s.interpolateHcl,c);return R`<image 
      x=${o}
      y=${n}
      width=${h-r-o}
      height=${d-n-a}
    ...
    ></image>`}renderDiscreteThreshold(){if("discrete"!==this.cle.scaleType&&"threshold"!==this.cle.scaleType)return"";const{tickSize:t,marginTop:i,marginLeft:e,colorScale:s,xScale:n}=this.cle,o=this.cle.height+t,r=this.cle.marginBottom+t,l=s.range();return R`${l.map((t=>R`<rect x=${(t=>s.invertExtent(t).map(n)[0]||e)(t)} y=${i} width=${(t=>{let[i,e]=s.invertExtent(t).map(n);return i=i||0,e=e||n.range()[1],e-i})(t)} height=${o-i-r} fill=${t}></rect>`))}`}renderAxis(){if(!this.cle.xScale||"categorical"===this.cle.scaleType)return"";const{ticks:t,tickSize:i,tickFormat:e,tickFormatter:s,tickValues:n,xScale:o,marginTop:r}=this.cle,l=this.cle.height+i,h=this.cle.marginBottom+i,c=(null==n?void 0:n.length)?n:o.ticks.apply(o,[t,e]),a=Math.max(i,0)+3;return R`<g
      class="x-axis"
      transform="translate(0, ${l-h})"
      text-anchor="middle"
    >${c.map((t=>R`<g class="tick" transform='translate(${o(t)},0)'>
